...
def beam_search_step_kernel(idx, act_n_sel,
    ...
    if prob1 > 1e-3 and prob2 > 1e-3:
        reward, new_graph_1, new_greedy,_,_ = \
            ged_env.step(graph_1_list[beam_idx], graph_2_list[beam_idx], ori_k_list[beam_idx], acts, orig_greedy,1)
        return (
                new_graph_1,
...
def beam_search_gfn(gfn, ged_env, inp_graph_1, inp_graph_2, ori_ks, greedy_cost, max_actions, beam_size=5, multiprocess_pool=None):
    start_time = time.time()

    state_encoder = gfn.state_encoder
    actor_net = gfn.forward_policy
...
def evaluate_gfn(gfn, ged_env, eval_graphs, max_steps=10, search_size=10, mp_pool=None):
    ret_result = {'reward': {}, 'ratio': {}, 'solution': {}, 'gap': {}, 'num_act': {}, 'time': {}}
    # Load test graphs
    for graph_index, (inp_graph_1, inp_graph_2, ori_k, ori_greedy, baselines, _) in enumerate(eval_graphs):
        # Running beam search:
        bs_result = beam_search_gfn(gfn, ged_env, inp_graph_1, inp_graph_2, ori_k, ori_greedy, max_steps, search_size, mp_pool)
        print(f'BEAMSEARCH \t'
              f'gid {graph_index} \t'
              f'time {bs_result["time"]:.2f} \t'
              f'reward {bs_result["reward"]:.4f} \t'
